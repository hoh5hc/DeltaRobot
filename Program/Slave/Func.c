...
float datarx;
#define BufferSize       6
uint8_t SPI_SLAVE_Buffer_Rx,startframe_rx,rx_spi[5];
uint8_t SPI_SLAVE_Buffer_Tx[BufferSize],spi_busy;
...
void PID_Position(void)
{
		...
		//SpeedPulse= -(((((double)(Pulse - PrePulse))*60/349/34)/4)/0.02);
		//PrePulse= Pulse;
...
void SPI1_IRQHandler(void)
{
  if(SPI_I2S_GetITStatus(SPI1,SPI_I2S_IT_RXNE)!=0)
  {
		...
		int i;
		spi_busy=1;
    SPI_SLAVE_Buffer_Rx=SPI_I2S_ReceiveData(SPI1);
		if(startframe_rx>=1)
		{
			rx_spi[startframe_rx-1]=SPI_SLAVE_Buffer_Rx;
			startframe_rx++;
		}
		...
		memcpy(&datarx, rx_spi, sizeof datarx); 
			if (rx_spi[4]==0 & datarx==0) // of code truyen du lieu
		{
    ...
    DMA_ClearITPendingBit(DMA1_IT_GL3);
		for (i=5;i>=1;i--)
		{
			SPI_SLAVE_Buffer_Tx[i]=SPI_SLAVE_Buffer_Tx[i-1];
		
		}
		SPI_SLAVE_Buffer_Tx[0]=0xff;
		DMA_Cmd(DMA1_Channel3, ENABLE);
		DMA_ITConfig(DMA1_Channel3, DMA_IT_TC, ENABLE);
		SPI_I2S_DMACmd(SPI1, SPI_I2S_DMAReq_Tx,ENABLE);
		}
		if (rx_spi[4]==1) // of code ramp time
		{
		if (datarx<=0)
			datarx=20;
		ramp_time=datarx;
		}
			if (rx_spi[4]==2) // ofcode reference pos
		{
			if (rxpos!=datarx)
			{
				if ( datarx==180)
